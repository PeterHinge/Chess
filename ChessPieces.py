...
class Queen(ChessPiece):
    ...
    def get_possible_moves(self):
        possible_moves = [[0, i+1] for i in range(7)] + [[0, -i-1] for i in range(7)] + \
                         [[i+1, 0] for i in range(7)] + [[-i-1, 0] for i in range(7)] + \
...
class Bishop(ChessPiece):
    ...
    def get_possible_moves(self):
        possible_moves = [[i+1, i+1] for i in range(7)] + [[i+1, -i-1] for i in range(7)] + \
                         [[-i-1, i+1] for i in range(7)] + [[-i-1, -i-1] for i in range(7)]
        for moves in possible_moves:
            moves[0] += self.x
            moves[1] += self.y
...
class Pawn(ChessPiece):
    ...
    def get_possible_moves(self):
        ...
        if self.direction is UP:
            for moves in possible_moves[0:3]:
                moves[0] += self.x
                moves[1] += self.y
            return possible_moves[0:3]
        elif self.direction is DOWN:
            for moves in possible_moves[3:6]:
                moves[0] += self.x
                moves[1] += self.y
